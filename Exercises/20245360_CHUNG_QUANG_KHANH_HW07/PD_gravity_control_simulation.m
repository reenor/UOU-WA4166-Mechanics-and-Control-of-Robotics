...
for i = 1:length(time)
    ...
    % Gravitational Torques (in Newton-meters)
    tau_g1 = -m1*g*(l1/2)*sin(deg2rad(theta1_actual)) - m2*g*(l1*sin(deg2rad(theta1_actual)) + (l2/2)*sin(deg2rad(theta1_actual + theta2_actual)));
    tau_g2 = -m2*g*(l2/2)*sin(deg2rad(theta1_actual + theta2_actual));
    
    % PD Control for Joint 1
    e1 = theta1_des - theta1_actual; % Position error
    e1_dot = theta1_dot_des - theta1_dot_actual; % Velocity error
    tau1 = Kp1*e1 + Kd1*e1_dot + tau_g1; % Control torque for Joint 1 (including gravity)
    
    % PD Control for Joint 2
    e2 = theta2_des - theta2_actual; % Position error
    e2_dot = theta2_dot_des - theta2_dot_actual; % Velocity error
    tau2 = Kp2*e2 + Kd2*e2_dot + tau_g2; % Control torque for Joint 2 (including gravity)
